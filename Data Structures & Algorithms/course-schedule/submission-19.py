class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        prereq = defaultdict(list)
        for course, pre in prerequisites:
            prereq[course].append(pre)
        cycle = set()   # List of all prerequesites cycle for the course, including it.
        def canBeTaken(course):
            if prereq[course] == []:   # If it has not prereqs, it can be cycle.
                return True
            if course in cycle:  # If there is a loop in courses, then that course cannot be cycle.
                return False
            cycle.add(course)
            for each in prereq[course]:
                if not canBeTaken(each):    # If a single prereq cant be cycle, course cant be cycle.
                    return False
            prereq[course] = [] # Removing the prerequsites for this course
            cycle.remove(course)
            return True
        for course in range(numCourses):
            if not canBeTaken(course):
                return False
        return True

            

        