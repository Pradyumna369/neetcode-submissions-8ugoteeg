class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        taken = set()    # List of all prerequesites taken for the course, including it. 
        def canBeTaken(course):
            if course in can:   # If it has not prereqs or it is already checked to be taken, it can be taken.
                return True
            if course in taken:  # If there is a loop in courses, then that course cannot be taken.
                return False
            taken.add(course)
